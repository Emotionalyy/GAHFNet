...
def train(train_loader, model, optimizer, epoch, train_save):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # ---- forward ----
            lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2, lateral_map_1, final_map, highmap, lateral_edge = model(
                images)  #
            # ---- loss function ----
        ...
        if i % 1 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], [lateral_map_1: {:.4f}, '
                  ...
                  format(datetime.now(), epoch, opt.epoch, i, total_step, loss_record1.show(),
                         ...
                         loss_record6.show(), loss_record7.show()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default=r'./Dataset_CLAHE/TrainingSet/Pseudo-label')
    ...
    if opt.backbone == 'mynet_VGG_CLAHE':
        print('Backbone loading: mynet_VGG_CLAHE')
    ...
    if opt.is_semi and opt.backbone == 'mynet_VGG_CLAHE':
        print('Loading weights from weights file trained on pseudo label')
        model.load_state_dict(torch.load('Snapshots/save_weights/mynet_VGG_CLAHE_Pseudo/mynet_VGG_CLAHE-100.pth'),
                              strict=False)
    else:
    ...
    if opt.is_pseudo and (not opt.is_semi):
        train_save = 'mynet_VGG_CLAHE_Pseudo'
    elif (not opt.is_pseudo) and opt.is_semi:
        train_save = 'Semi-mynet_VGG_CLAHE'
    elif (not opt.is_pseudo) and (not opt.is_semi):
        train_save = 'mynet_VGG_CLAHE'
    else:
    ...
    plt.axis([0, 100, 0, 2])
    plt.title("Loss after epoch 100 VGG")
    plt.xlabel("epoch")
    plt.ylabel("Loss")
    plt.grid(alpha=0.4)
    plt.legend()
