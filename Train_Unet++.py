...
if __name__ == '__main__':
    ...
    model = UNet_2Plus(in_channels=opt.net_channel, n_classes=opt.n_classes, feature_scale=4, is_deconv=True,
                       is_batchnorm=True, is_ds=True).cuda()

    if opt.is_semi and opt.backbone == 'UNet++':
        print('Loading weights from weights file trained on pseudo label')
        model.load_state_dict(torch.load('./Snapshots/save_weights/UNet++_Pseduo/UNet++-100.pth'), strict=False)
    else:
