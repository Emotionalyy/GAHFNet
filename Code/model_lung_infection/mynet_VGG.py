...
class mynetVGG(nn.Module):
    ...
    def forward(self, x):
        ...
        highmap = F.interpolate(high_map, scale_factor=4,
                                mode='bilinear')  # 损失函数中的高层指导 NOTES: Sup-1 (bs, 1, 88, 88) -> (bs, 1, 352, 352)
        ...
        x_hm_1 = torch.cat(
            (self.ra_h1_conv1(x_hm_1), F.interpolate(edge_guidance, scale_factor=1 / 16, mode='bilinear')), dim=1)
        x_hm_1 = F.relu(self.ra_h1_conv2(x_hm_1))
        high_map_5 = self.ra_h1_conv3(x_hm_1)
        x_5 = high_map_5 + hm_1  # element-wise addition
        lateral_map_5 = F.interpolate(x_5, scale_factor=16,
        ...
        x_hm_2 = torch.cat(
            (self.ra_h2_conv1(x_hm_2), F.interpolate(edge_guidance, scale_factor=1 / 8, mode='bilinear')), dim=1)
        x_hm_2 = F.relu(self.ra_h2_conv2(x_hm_2))
        high_map_4 = self.ra_h2_conv3(x_hm_2)
        x_4 = high_map_4 + hm_2
        lateral_map_4 = F.interpolate(x_4, scale_factor=8,
        ...
        x_mm_2 = torch.cat(
            (self.ra_m2_conv1(x_mm_2), F.interpolate(edge_guidance, scale_factor=1 / 4, mode='bilinear')), dim=1)
        ...
        x_lm_1 = torch.cat(
            (self.ra_l1_conv1(x_lm_1), F.interpolate(edge_guidance, scale_factor=1 / 2, mode='bilinear')), dim=1)
        x_lm_1 = F.relu(self.ra_l1_conv2(x_lm_1))
        low_map_2 = self.ra_l1_conv3(x_lm_1)
        x_2 = low_map_2 + lm_1
        lateral_map_2 = F.interpolate(x_2, scale_factor=2,
        ...
        final_map = lateral_map_5 + lateral_map_4 + lateral_map_3 + lateral_map_2 + lateral_map_1

        # 特征融合
        return lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2, lateral_map_1, final_map, \
