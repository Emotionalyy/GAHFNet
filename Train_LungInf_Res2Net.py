...
def train(train_loader, model, optimizer, epoch, train_save):
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        if i == total_step:
            l1.append(loss1.detach().cpu().numpy())  # S1
            l2.append(loss2.detach().cpu().numpy())  # S2
            l3.append(loss3.detach().cpu().numpy())  # S3
            l4.append(loss4.detach().cpu().numpy())  # S4
            l5.append(loss5.detach().cpu().numpy())  # S5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='./Dataset/TrainingSet/Pseudo-label')
    ...
    for epoch in tqdm(range(1, opt.epoch)):
        ...
        epoch1.append(epoch)
    plt.plot(epoch1, l1, label="Eg")
    plt.plot(epoch1, l2, label="S3")
    plt.plot(epoch1, l3, label="S4")
    plt.plot(epoch1, l4, label="S5")
    plt.plot(epoch1, l5, label="Sg")

    plt.axis([0, 100, 0, 3])
    plt.title("Loss after epoch 100 Res2Net")
    plt.xlabel("epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.show()
